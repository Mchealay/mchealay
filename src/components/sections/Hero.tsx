...
import { Mail, Download, ExternalLink, ArrowRight, Zap } from 'lucide-react';
      ...
      <div className="relative z-10 max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-24 text-center">
        ...
        >
          ...
          >
            ...
            <span className="text-cyan-400 font-medium">high-performance systems</span> that scale
            to millions of users — from zero-to-one products to mission-critical infrastructure.
            Obsessed with clean code, developer experience, and measurable impact.
          </motion.p>

          {/* Stats chips */}
          <motion.div variants={itemVariants} className="flex flex-wrap items-center justify-center gap-3">
            {[
              { icon: <Zap className="w-3.5 h-3.5" />, text: '5+ Years Experience' },
              { icon: <ExternalLink className="w-3.5 h-3.5" />, text: '30+ Projects Shipped' },
              { icon: <ArrowRight className="w-3.5 h-3.5" />, text: 'Open Source Contributor' },
            ].map((chip) => (
              <span
                key={chip.text}
                className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-white/5 border border-white/10 text-xs text-slate-400"
              >
